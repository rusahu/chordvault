...
const db = new Database(process.env.DB_PATH || './data/chordvault.db');
...
// pinyin/tags columns, drop it + its triggers so the CREATE below rebuilds the
// new schema and the empty-table backfill repopulates every searchable field.
{
  const ftsRow = db.prepare("SELECT sql FROM sqlite_master WHERE type='table' AND name='songs_search'").get();
  if (ftsRow && (!/\bpinyin\b/.test(ftsRow.sql) || !/\btags\b/.test(ftsRow.sql))) {
    db.exec(`
...
db.exec(`
  ...
  CREATE VIRTUAL TABLE IF NOT EXISTS songs_search USING fts5(
    title, artist, lyrics, tags, pinyin, tokenize='trigram'
  );

  -- Synchronization Triggers
  CREATE TRIGGER IF NOT EXISTS songs_search_insert AFTER INSERT ON songs BEGIN
    INSERT INTO songs_search(rowid, title, artist, lyrics, tags, pinyin)
    VALUES (new.id, search_text(new.title), search_text(new.artist), search_lyrics(new.content), search_text(new.tags),
...
// Backfill index if empty
{
  ...
  if (count === 0) {
    db.prepare(`
      INSERT INTO songs_search(rowid, title, artist, lyrics, tags, pinyin)
      SELECT id, search_text(title), search_text(artist), search_lyrics(content), search_text(tags),
...
// Migrate setlist entries from an accumulated transpose delta to a stored target key
{
  const { migrateTargetKey } = require('./targetKeyMigration');
  migrateTargetKey(db);
}
