...
const getEntriesStmt = db.prepare(`
  SELECT ss.id as entry_id, ss.song_id, ss.position, ss.target_key, ss.nashville, ss.font, ss.two_col, ss.content_override,
...
const addSongEntryTransaction = db.transaction((setlistId, songId, targetKey, nashville) => {
  const maxPos = db.prepare('SELECT MAX(position) as max FROM setlist_songs WHERE setlist_id = ?').get(setlistId);
  const position = (maxPos.max || 0) + 1;
  const result = db.prepare('INSERT INTO setlist_songs (setlist_id, song_id, position, target_key, nashville) VALUES (?, ?, ?, ?, ?)')
    .run(setlistId, songId, position, targetKey || null, nashville ? 1 : 0);
...
const updateSongEntryTransaction = db.transaction((entryId, setlistId, entry, { targetKey, targetKeyProvided, nashville, font, twoCol, contentOverride }) => {
  db.prepare(`UPDATE setlist_songs SET
    ...
    targetKeyProvided ? 1 : 0,
    targetKeyProvided ? (targetKey ?? null) : null,
...
module.exports = {
  ...
  addSongEntry: (setlistId, songId, { targetKey, nashville }) => {
    return addSongEntryTransaction(setlistId, songId, targetKey, nashville);
  },
