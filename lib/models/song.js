...
const selectUserTagsStmt = db.prepare('SELECT tags FROM songs WHERE user_id = ? AND status = ? AND tags IS NOT NULL');
const selectPublicTagsStmt = db.prepare('SELECT tags FROM songs WHERE visibility = ? AND status = ? AND tags IS NOT NULL');
...
module.exports = {
  ...
  // Dynamic searches
  listForUser(userId, { q, language, tags = [], page = null, limit = null } = {}) {
    let selectPart = `
    ...
    for (const tag of tags) {
      bodyPart += " AND instr(',' || lower(s.tags) || ',', ',' || lower(?) || ',') > 0";
      bodyParams.push(tag);
    }
  ...
  listTagsForUser(userId) {
    const tags = normalizeTags(selectUserTagsStmt.all(userId, STATUS.ACTIVE).flatMap((row) => String(row.tags).split(',')));
    return tags.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  },

  listPublic({ q, language, tags = [], userId = 0, page = null, limit = null } = {}) {
    let selectPart = `
  ...
  listPublicTags() {
    const tags = normalizeTags(selectPublicTagsStmt.all(VISIBILITY.PUBLIC, STATUS.ACTIVE).flatMap((row) => String(row.tags).split(',')));
    return tags.sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  },
