 ...
 * Text query hits title/artist/lyrics/tags; a space-stripped copy hits the pinyin
...
function getFtsSearch(q) {
  ...
  const textBranch = forms.map((f) => `{title artist lyrics tags}:(${ftsMatchExpr(f)})`).join(' OR ');
 ...
 * term hits: title (3) > artist (2) > tags (1) > lyrics-only (0). Recency stays as tiebreak.
...
function getRelevanceOrder({ qClean, qTrad, qSimp, hasVariants }) {
  ...
  return {
    orderBy: `MAX(CASE WHEN ${cond('s.title')} THEN 3 WHEN ${cond('s.artist')} THEN 2 WHEN ${cond('s.tags')} THEN 1 ELSE 0 END) DESC, `,
    orderParams: [...likes, ...likes, ...likes],
  };
}
 ...
 * matching the FTS scope (title + artist + lyrics + tags) exactly. Trad/Simp variants
...
function getShortSongSearch({ qClean, qTrad, qSimp, hasVariants }) {
  const cols = ['ss.title', 'ss.artist', 'ss.lyrics', 'ss.tags'];
  const forms = hasVariants ? [qTrad, qSimp] : [qClean];
